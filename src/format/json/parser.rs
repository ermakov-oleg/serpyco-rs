...
impl<'j> JsonParser<'j> {
    ...
    /// Raw text of a number (for Decimal / manual int-vs-float split — no
    /// precision loss), already positioned + peeked by a preceding `peek()`.
    #[inline]
    ...
    /// Enter an object, self-positioning (peeks internally). For the one caller
    /// that reads an object without a preceding `peek()` — the discriminated
    /// union's throwaway scan sub-parser.
