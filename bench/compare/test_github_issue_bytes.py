...
# The sibling module is literally named ``msgspec``; alias it on import so it
# never shadows the top-level ``msgspec`` package.
from .github_issue import mashumaro, msgspec as msgspec_issue, serpyco_rs, serpyco_rs_codec


# Each contender decodes the raw payload with its own ``load`` and encodes with
# its own ``dump``; the reference object is derived per contender in the tests.
CONTENDERS = {
    'serpyco_rs_codec': {
    ...
    'msgspec': {
        'dump': msgspec_issue.dump,
        'load': msgspec_issue.load,
        'skip_refcount': True,
    },
    'mashumaro+orjson': {
